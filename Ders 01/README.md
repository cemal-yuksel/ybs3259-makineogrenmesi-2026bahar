...
```mermaid
graph LR
    A[❌ Eski Bakış] --> B[🔄 Paradigma Değişimi]
    B --> C[✅ Yeni Bakış]
    
    A1[Kod yazmak] --> A
    A2[Algoritma ezberlemek] --> A
    A3[Teknik detaylar] --> A
    
    C --> C1[Karar vermek]
    C --> C2[İş değeri yaratmak]
    C --> C3[Stratejik düşünmek]
...
```mermaid
graph TD
    A[🎯 DERS 01 DNA'sı] --> B[💼 İş Odaklılık]
    A --> C[🧠 Kavramsal Derinlik]
    A --> D[🌍 Gerçek Dünya]
    
    B --> B1[CEO senaryoları]
    B --> B2[YBS perspektifi]
    B --> B3[Karar verme]
    
    C --> C1[AI-ML-DL hiyerarşisi]
    C --> C2[Öğrenme türleri]
    C --> C3[Metodoloji]
    
    D --> D1[Netflix örneği]
    D --> D2[Kaggle/UCI]
    D --> D3[Endüstri uygulamaları]
...
```mermaid
graph LR
    A[🤔 Eski Algı] --> B[💡 Dönüşüm] --> C[✨ Yeni Algı]
    
    A1["ML = Kod yazmak"] --> A
    A2["YBS = IT destek"] --> A
    
    C --> C1["ML = Karar aracı"]
    C --> C2["YBS = Strateji mimarı"]
...
```mermaid
graph TD
    A[📊 Ham Veri] --> B{ML Motoru}
    B --> C[🔍 Örüntü Keşfi]
    C --> D[📈 Model]
    D --> E[🔮 Öngörü]
    
    E --> F1[Bu müşteri risk altında]
    E --> F2[Bu ürün satacak]
    E --> F3[Bu kampanya başarılı olur]
...
```mermaid
graph LR
    A[Geçmiş<br/>Tecrübe] --> B[Sezgi]
    C[Geçmiş<br/>Veri] --> D[Örüntü]
    
    B --> E{Karar}
    D --> E
    
    E --> F[Gelecek Aksiyonu]
...
```mermaid
timeline
    title AI/ML/DL Tarihsel Gelişim
    1950 : AI Kavramı (Turing)
    1980 : Uzman Sistemler Çağı
    1990 : ML Algoritmaları (SVM, Decision Trees)
    2006 : Derin Öğrenme Patlaması (Hinton)
    2012 : ImageNet Devrimi (AlexNet)
    2017 : Transformer Mimarisi (Attention)
    2022 : ChatGPT ve LLM Çağı
```
...
```mermaid
graph TD
    A[🤖 Model Çıktısı] --> B{YBS Analizi}
    B --> C[📊 İş Bağlamı]
    B --> D[💰 Maliyet-Fayda]
    B --> E[⚖️ Risk Değerlendirmesi]
    
    C --> F[🎯 Aksiyon Planı]
    D --> F
    E --> F
    
    F --> G[Segment 1: Premium Kampanya]
    F --> H[Segment 2: Sadakat Programı]
    F --> I[Segment 3: Anket + İndirim]
...
```mermaid
graph LR
    A[🎬 İçerik] --> B[📸 100+ Thumbnail]
    C[👤 Kullanıcı Profili] --> D[🤖 ML Modeli]
    B --> D
    D --> E[🎯 En İyi Thumbnail]
    E --> F[👁️ Tıklama Oranı ↑]
...
```mermaid
graph TD
    A[🤔 İş Sorusu] --> B{Geçmişte cevabı<br/>bildiğim örnekler<br/>var mı?}
    
    B -->|Evet| C[📚 DENETİMLİ ÖĞRENME]
    B -->|Hayır| D[🔍 DENETİMSİZ ÖĞRENME]
    
    C --> C1{Sonuç<br/>sayısal mı?}
    C1 -->|Evet| C2[📈 REGRESYON]
    C1 -->|Hayır| C3[🎯 SINIFLANDIRMA]
    
    D --> D1{Gruplar<br/>bulmak mı?}
    D1 -->|Evet| D2[🧩 KÜMELEME]
    D1 -->|Hayır| D3[🔗 BİRLİKTELİK]
...
```mermaid
graph TD
    A[1️⃣ İş Anlayışı] --> B[2️⃣ Veri Anlayışı]
    B --> C[3️⃣ Veri Hazırlığı]
    C --> D[4️⃣ Modelleme]
    D --> E[5️⃣ Değerlendirme]
    E --> F[6️⃣ Canlıya Alma]
...
```mermaid
graph TD
    A[İş Problemi] --> B{Problem Türü?}
    B -->|Sınıflandırma| C[Model Adayları]
    B -->|Regresyon| D[Model Adayları]
    ...
    E --> F{En İyi?}
    F -->|Evet| G[Hiperparametre Optimizasyonu]
    F -->|Hayır| E
```
...
```mermaid
graph LR
    A[CRM Sistemi] --> B[API Gateway]
    B --> C[ML Model Service]
    C --> D[Tahmin]
    D --> E[CRM'e Geri Dön]
    E --> F[Pazarlama Automation]
...
```mermaid
graph TD
    A[🎯 Veri Kaynakları] --> B[🏢 Kurumsal Veri]
    A --> C[🌐 Açık Veri Platformları]
    A --> D[💰 Ticari Veri Sağlayıcılar]
    
    B --> B1[İç Sistemler: CRM, ERP]
    B --> B2[İşlem Veritabanları]
    B --> B3[Log Dosyaları]
    
    C --> C1[⭐ Kaggle]
    C --> C2[⭐ UCI]
    C --> C3[Google Dataset Search]
    C --> C4[Data.gov]
    C --> C5[GitHub Public Datasets]
    
    D --> D1[Bloomberg Terminal]
    D --> D2[Nielsen]
    D --> D3[IDC Research]
...
```mermaid
graph TD
    A[1️⃣ İş Problemi Tanımla] --> B[2️⃣ Veri İhtiyacı Belirle]
    B --> C[3️⃣ Kaynak Araştır]
    C --> D[4️⃣ Kalite Kontrolü]
    D --> E[5️⃣ Lisans Kontrol]
    E --> F{Uygun mu?}
...
[![For YBS Students](https://img.shields.io/badge/For-YBS%20Students-blue.svg?style=for-the-badge)]()
[![ML Journey](https://img.shields.io/badge/ML-Journey%20Begins-green.svg?style=for-the-badge)]()
